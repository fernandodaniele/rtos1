...
int main( void )
{
	...
	debugPrintlnString( "Ejercicio C_4.\r\n" );
	...
	{
		gpioWrite( LEDR, ON );
		debugPrintlnString( "Error al crear los semaforos.\r\n" );
		while(TRUE);						// VER ESTE LINK: https://pbs.twimg.com/media/BafQje7CcAAN5en.jpg
	}
...
void tarea_led( void* taskParmPtr )
{
	...
	TickType_t xPeriodicity =  5000 / portTICK_RATE_MS;
	// ---------- REPETIR POR SIEMPRE --------------------------
    while( TRUE )
    {
    	if (xSemaphoreTake( sem_tec_pulsada , 0 )==pdTRUE)			// Esperamos tecla
    	{
    		gpioWrite( LEDG , ON );
    		vTaskDelay( LED_RATE );
    		gpioWrite( LEDG , OFF );
