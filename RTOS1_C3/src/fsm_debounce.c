...
void buttonReleased( void )
{
	...
	if (tiempo_diff > 0)
	{
		//printf(uxSemaphoreGetCount(sem_tec_pulsada)); para saber cuantos hay, aca daria 0
		xSemaphoreGive(  sem_tec_pulsada ); // contador++;
		xSemaphoreGive(  sem_tec_pulsada );
		xSemaphoreGive(  sem_tec_pulsada );
		//printf(uxSemaphoreGetCount(sem_tec_pulsada)); para saber cuantos hay, aca daria 3
	}
